...
const NETWORK_TYPE_WIFI        = nsINetworkInfo.NETWORK_TYPE_WIFI;
const NETWORK_TYPE_MOBILE      = nsINetworkInfo.NETWORK_TYPE_MOBILE;
...
let gRadioEnabledController = (function() {
  let _ril = null;
  ...
  return {
    ...
    notifyRadioStateChanged: function(clientId, radioState) {
      //XXX vmx 2017-06-10: GO ON HERE an make gMobileConnectionsService (gecko-dev/dom/mobileconnection/gonk/MobileConnectionService.js) working. This service sends a `setRadioEnabled` message to the worker which will enable the radio.
...
DataCall.prototype = {
  failCause: nsIDataCallInterface.DATACALL_FAIL_NONE,
...
// TODO vmx 2017-05-26: Implement `Service` properly
...
RadioInterfaceLayer.prototype = {
  ...
  get numRadioInterfaces() {
    // TODO vmx 2017-05-26: Implement `Service` properly. For now, just
    // return `1` to get thing working.
...
function WorkerMessenger() {
  ...
  this.worker = new WebSocket('ws://localhost:3012');
  this.worker.onerror = this.onerror.bind(this);
  this.worker.onmessage = this.onmessage.bind(this);
  this.worker.onopen = this.init.bind(this);
}
WorkerMessenger.prototype = {
  ...
  registerClient: function(aClientId, aRadioInterface) {
    ...
    // NOTE vmx 2017-05-26: I don't think that's needed anymore, I'm not sure
    // though, hence leaving it here commented out
    //gSystemWorkerManager.registerRilWorker(aClientId, this.worker);
  },

  // This is a wrapper that will wait until the WebSocket connection is
  // established before executing the readl send function. Once the connection
  // is established only the read send function will be executed.
  send: function(...args) {
    if (this.worker.readyState === WebSocket.CONNECTING) {
      window.setTimeout(() => {
        this.send(...args)
      }, 100);
    } else {
      // Redefine `send` with the real implementation
      this.send = this.realSend;
      this.send(...args);
    }
  },
   ...
   */
  realSend: function(clientId, rilMessageType, message, callback) {
    message = message || {};
...
// TODO vmx 2017-06-04: I'm leaving the gSettingsService out for now, but
// I guess it should eventually get implemented.
